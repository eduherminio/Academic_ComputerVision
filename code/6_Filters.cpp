...
int main( int argc, char* argv[])
{
  ...
  std::vector<Mat>          v_Pic_dst;
  std::vector<std::string>  v_picName;

  v_Pic_grey.reserve(argc-1);
  v_Pic_dst.reserve(argc-1);
  ...
  for(int i=0; i<argc-1; ++i)  {
    fastNlMeansDenoisingColored(v_Pic[i], v_Pic_dst[i], h, hColor, templateWindowSize, searchWindowSize);
        std::string a(std::to_string(i));
    show_pic(v_Pic_dst[i], "Without "+a);
  }

  h=11;
  fastNlMeansDenoisingColored(v_Pic[argc-2], v_Pic_dst[argc-2], h, hColor, templateWindowSize, searchWindowSize);
  show_pic(v_Pic_dst[argc-2], "Without noise - h=11");
